...
return_path = '/Users/Momooo/Dropbox/transfer_cityu_data/Return/5DForward.csv'
...
for j in range(0,5):
    ...
    while order < len(alldates):
        ...
        if order > 5 + j:
            ...
            distance = svc.decision_function(mydata)
            distance = pd.DataFrame(distance)
            distance.columns = [current_date]
            distance.index = myindex
            #print("####distance: ",distance)
            #save the factor value
            factor_results = pd.concat([factor_results,distance],axis=1)
            print("#result shape:::" ,factor_results.shape)
            
            
            '''
            distance = distance.sort_values(by=current_date,ascending = False)
            #print("distance: ",probs['distance'])
    
            positive = mydata.loc[distance.index[:10]]
            negative = mydata.loc[distance.index[-10:]]
            print("positive: ",positive.index)
            print("negative: ",negative.index)
            ...
            print("pos: ",positive_return,"neg: ",negative_return)
            ...
            validreturns.append(current_return+1)
        ...
        #####set model and train
        svc = svm.SVC(C=1)
        current_result = svc.fit(traindata,target)
